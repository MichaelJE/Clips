...
Highlighter::Highlighter(Config* config, CodeEditor* codeEditor)
    : QSyntaxHighlighter(reinterpret_cast<QTextEdit*>(codeEditor)), config(config), codeEditor(codeEditor)
{
    ...
    disconnect(document(), SIGNAL(contentsChange(int, int, int)), this, SLOT(_q_reformatBlocks(int, int, int)));
}
